...
def dataframe():
    ...
    namelist = list(it.product(*namelist))
    namelist = pd.DataFrame(namelist, columns = ["Driver", "Body", "Tires", "Glider"])
    ...
    statlist = list(it.product(*statlist))
    ...
    namelist["Total"] = namelist.sum(axis=1)
